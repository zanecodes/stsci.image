...
def blkavg(infiles,outfiles,extension_num=None,b1=1,b2=1,b3=1):
    ...
    for infile in infile_split:
        ...
        if b3 > 1:
            ...
            x_bin = float(x_arr_dim) / b1

            y_arr_dim = scidata.shape[1]
            y_bin = float(y_arr_dim) / b2

            z_arr_dim = scidata.shape[0]
            z_bin = float(z_arr_dim) / b3

            outarr = np.zeros((int(math.ceil(z_bin)),int(math.ceil(y_bin)),int(math.ceil(x_bin))))
            ...
            outarr = np.zeros((int(math.ceil(y_bin)),int(math.ceil(x_bin))))
        ...
        for i,x1 in enumerate(range(0,x_arr_dim,b1)):
            ...
            for j,y1 in enumerate(range(0,y_arr_dim,b2)):
                ...
                if b3 == 1:
                    ...
                    for k,z1 in enumerate(range(0,z_arr_dim,b3)):
                        ...
                        outarr[k,j,i] = scidata[z1:z2,y1:y2,x1:x2].mean()
...
def update_hdr(card_list,b1,b2,b3):
    ...
    ltv1 = (np.median(np.array([1,b1])) + LTV1 - 1) / b1
    ltv2 = (np.median(np.array([1,b2])) + LTV2 - 1) / b2
    ratio_x = b1 / VAFACTOR
    ratio_y = b2 / VAFACTOR
